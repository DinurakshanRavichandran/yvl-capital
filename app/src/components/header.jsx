...
const Header = () => {
  ...
  const navLinks = [
    { name: "Home", href: "#home", type: "hash" }, // Home option added back
  ...
  const handleNavClick = (e, link) => {
    if (link.type === "hash") {
      ...
      if (location.pathname !== "/") {
        // If on /team, go home first then scroll/top
        navigate("/");
        setTimeout(() => {
          if (link.href === "#home") {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden xl:flex items-center gap-1">
          {navLinks.map((link) => (
            <a
              ...
              className="relative px-4 py-2 text-[12px] uppercase tracking-[0.2em] text-white/50 hover:text-[#E8A147] transition-all duration-300 group"
            >
              {link.name}
        ...
        <div className="hidden md:block">
          <a
            ...
            className="relative px-7 py-3 rounded-full overflow-hidden group inline-block"
          >
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`xl:hidden absolute top-full left-0 right-0 bg-[#030210]/95 backdrop-blur-xl border-b border-[#E8A147]/10 overflow-hidden transition-all duration-500 ${
